...
public class DeviceGroupController {
    ...
    public CommonResultVo<List<DeviceGroup>> findGroup(@Valid DeviceIdDto dto) {
       ...
       // String gatway = dto.getDeviceId().substring(dto.getDeviceId().length() - 2);
        ...
        DeviceGroup build = DeviceGroup.builder().userId(uId).build();
        //查询主控下的组数据
        //查询当前设备
        UserDevice byId2 = userDeviceService.getById(dto.getDeviceId());

        ValidUtils.isNullThrow(byId2, "设备未绑定");
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            //ValidUtils.listIsEmptyThrow(list1, "DeviceGroupList数据不存在"); 测试说不返回
            if(list1.size() == 0){
            ...
            deviceGroup.setDeviceId(list1.get(0).getDeviceId());
