...
@Slf4j
@Component
public class HwLastReminderSendVoiceJob extends AbstractApplicationContextJob {
    @Autowired
    ...
    public void execute(JobExecutionContext context) {
        ...
        String orderNo = jobDataMap.getString(QuartzUtil.KEY_OF_LAST_REMINDER_SEND_VOICE);
        SendVoiceDto sendVoiceDto = SendVoiceDto.builder().code("4").orderNo(orderNo).build();
        CommonResultVo<String> commonResultVo = hotelWechatApiFeignClient.roomSendVoice(sendVoiceDto);
        log.error("-----Hw Last Reminder Send Voice Job-------execute---sendVoiceDto={},commonResultVo={}",
