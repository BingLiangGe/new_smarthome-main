 ...
 */
@Slf4j
public class ClockJob extends AbstractApplicationContextJob {
    ...
    public void execute(JobExecutionContext context) {
        ...
        long id = jobDataMap.getLong(QuartzUtil.KEY_OF_CLOCK_ID);

        log.info("ClockJob.execute{}", JSON.toJSONString(jobDetail));
        CommonResultVo<String> commonResultVo = appApiFeignClient.triggerClock(IdDto.builder().id(id).build());
        log.info("ClockJob.commonResultVo={}", commonResultVo);
