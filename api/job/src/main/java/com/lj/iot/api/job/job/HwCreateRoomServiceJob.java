...
@Slf4j
@Component
public class HwCreateRoomServiceJob extends AbstractApplicationContextJob {
    @Autowired
    ...
    public void execute(JobExecutionContext context) {
        ...
        String orderNo = jobDataMap.getString(QuartzUtil.KEY_OF_ORDER_ROOM_SERVICE_JOB);

        SendVoiceDto sendVoiceDto = SendVoiceDto.builder().orderNo(orderNo).build();
        CommonResultVo<String> commonResultVo = hotelWechatApiFeignClient.createRoomService(sendVoiceDto);
        log.error("-----Hw Create Room Service Job-------execute-----orderNo={},commonResultVo={}",
                orderNo, JSONObject.toJSONString(commonResultVo));
