...
public class SosContactController {
    ...
    public CommonResultVo<SosContact> add(@RequestBody @Valid SosContactAddDto dto) {
        ...
        Home home = homeService.getOne(new QueryWrapper<>(Home.builder().id(dto.getHomeId()).userId(UserDto.getUser().getActualUserId()).build()));
        ValidUtils.isNullThrow(home, "数据不存在");
        SosContact sosContact = SosContact.builder().userId(UserDto.getUser().getActualUserId()).homeId(home.getId()).contactType(contactTypeEnum.getCode()).phoneNumber(dto.getPhoneNumber()).username(dto.getUsername()).build();
    ...
    public CommonResultVo<SosContact> edit(@RequestBody @Valid SosContactEditDto dto) {
        ...
        SosContact sosContact = sosContactService.getOne(new QueryWrapper<>(SosContact.builder().id(dto.getId()).userId(UserDto.getUser().getActualUserId()).build()));
