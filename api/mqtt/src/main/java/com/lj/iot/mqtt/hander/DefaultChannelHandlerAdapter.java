package com.lj.iot.mqtt.hander;

import com.lj.iot.mqtt.MqttRequest;
import com.lj.iot.mqtt.auth.IAuthService;
import com.lj.iot.mqtt.cache.IQ;
...
@Component
public class DefaultChannelHandlerAdapter extends ChannelInboundHandlerAdapter {
    ...
    @Autowired
    private IPingPongService pingPongService;
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        if (msg instanceof MqttMessage) {
            ...
            switch (messageType) {
                // 建立连接
                case CONNECT:
                    this.connect(ctx, (MqttConnectMessage) message);
                    break;
                // 发布消息
                case PUBLISH:
     ...
     */
    public void pingBack(Channel channel, MqttMessage msg) {
        pingPongService.handler(channel, msg);
        back(channel, MqttMessageType.PINGRESP);
    }

    public void back(Channel channel, MqttMessageType messageType) {
        MqttMessage mqttMessage = new MqttMessage(new MqttFixedHeader(messageType, false,
...
//            channel.writeAndFlush(mqttMessage);
    ...
    public void publish(Channel channel, MqttPublishMessage msg) {
        ...
        if (MqttQoS.AT_LEAST_ONCE == msg.fixedHeader().qosLevel() || MqttQoS.AT_MOST_ONCE == msg.fixedHeader().qosLevel()) {
            for (Channel channel1 : clientList) {
                try {
